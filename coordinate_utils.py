...
def return_coordinates_double_pendulum_controller(double_pendulum, inits, ts, method='rk4', noise=0., old_ver=False):
    ...
    if isinstance(double_pendulum, DoublePendulumApproxDiffEq) and double_pendulum.controller is not None:
        ...
        if old_ver:
            data = 2. * (1.01 + double_pendulum.controller(data_controller))
    ...
    x1 = np.sin(coord[:, :, 0])  # data[2, :, :] *
    # length_1 * cos(theta_1)
    y1 = np.cos(coord[:, :, 0])  # data[2, :, :] *
    # # x1 + length_2 * sin(theta_2)
    x2 = x1 + np.sin(coord[:, :, 1])  # data[3, :, :] *
    # # y1 + length_2 * cos(theta_2)
    y2 = y1 + np.cos(coord[:, :, 1])  # data[3, :, :] *
